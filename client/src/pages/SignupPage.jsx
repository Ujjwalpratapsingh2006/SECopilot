...
const SignupPage = () => {
  ...
  // Redirect automatically if they are already logged in
  useEffect(() => {
    if (user && !authLoading) {
    ...
    <div className="min-h-screen bg-sec-dark bg-grid-pattern relative overflow-hidden font-sans flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md p-8">
        ...
        <div className="bg-white/[0.03] border border-white/10 backdrop-blur-md rounded-2xl p-8 shadow-2xl">
          ...
          <p className="text-gray-400 text-center mb-8">Join SECopilot to decode Wall Street.</p>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              <input 
                ...
                required
                className={`w-full px-4 py-3 rounded-xl bg-white/5 border ${invalidFields.includes('username') ? 'border-sec-red focus:ring-sec-red/20' : 'border-white/10 focus:border-sec-green focus:ring-sec-green/20'} text-white placeholder-gray-500 focus:outline-none focus:ring-4 transition-all`}
              />
            </div>
          ...
          <div className="mt-6 text-center text-gray-400 text-sm">
            ...
            <Link to="/login" className="text-sec-green hover:text-emerald-400 font-semibold transition-colors">
              Log in
            </Link>
          </div>
        </div>
